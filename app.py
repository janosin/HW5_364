...
app.config["SQLALCHEMY_DATABASE_URI"] = "postgresql://localhost/janosinHW5"
## Provided:
...
class UpdatePriorityForm(FlaskForm):
    name = StringField("What is the new priority of this item?", validators=[Required()]) #integer 
    submit = SubmitField("Submit")
...
def update(item):
    ...
    if form.validate_on_submit():
        if request.method == 'POST':
            name = form.name.data 
            TodoItem.query.filter_by(id=item).first().priority = int(name)
            i = TodoItem.query.filter_by(id=item).first()
            db.session.commit()
            flash("Updated priority of " + i.description + " to " + str(name))
            return redirect(url_for('all_lists'))
...
def delete(lst):
    form = DeleteButtonForm()
    if request.method == 'POST':
        lt = TodoList.query.filter_by(id=lst).first()
        db.session.delete(lt)
        db.session.commit()
        new_lists = TodoList.query.all()
        flash('Deleted list ' + lst)
        
    return redirect(url_for('all_lists', form=form, todo_lists=new_lists))
